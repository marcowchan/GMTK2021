...
void UAkMediaAsset::LoadAndSetMedia(bool bLoadAsync)
{
	...
	if (LoadRefCount.GetValue() == 0)
	{
		...
		{
			AssetData->Load(bLoadAsync, DoSetMedia);
		}
	}
	else
	{
...
void UAkMediaAsset::UnloadMedia(bool bForceUnload /* = false */)
{
	auto AssetData = getMediaAssetData();
	if (!AssetData)
	{
	...
	if (LoadRefCount.GetValue() > 0)
	{
		...
		if (LoadRefCount.GetValue() <= 0 || bForceUnload)
		{
			if (AssetData->IsStreamed)
			{
			...
			if (AssetData->DataChunks.Num() > 0)
			{
				if (!AssetData->IsStreamed || (AssetData->IsStreamed && AssetData->DataChunks[0].IsPrefetch))
				{
